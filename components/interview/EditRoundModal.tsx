...
type EditRoundModalProps = {
    isOpen: boolean;
    onClose: () => void;
    roundData: EditRoundFormData;
    maxRoundNumber: number;
    onSubmit: (data: EditRoundFormData) => void;
};

export type EditRoundFormData = {
    _id: string;
    roundNumber: number;
    roundType: string;
...
export function EditRoundModal({ isOpen, onClose, onSubmit, roundData, maxRoundNumber }: EditRoundModalProps) {
    const [formData, setFormData] = useState<EditRoundFormData>(roundData);
    useEffect(() => {
        setFormData(roundData);
    }, [roundData]);

    const handleChange = (field: keyof EditRoundFormData, value: string | number) => {
                    ...
                    >
                        <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-elevated">
                            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                                <h2 className="text-lg font-semibold text-gray-900">Edit Round</h2>
                                <button
                                    onClick={onClose}
